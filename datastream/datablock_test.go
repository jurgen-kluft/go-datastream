...
func TestWriteNilPtrDoesNotRegisterRelocation(t *testing.T) {
	for _, pointerSize := range []SizeOfPointer{SizeOfPointer32, SizeOfPointer64} {
		...
		if got := block.Writer.Bytes(); !bytes.Equal(got, make([]byte, int(pointerSize))) {
			t.Fatalf("pointer size %d encoded nil as %v", pointerSize, got)
		}
		block.finalize(ctx, 0, nil, nil, true)
		if ctx.HasErrors() {
			t.Fatalf("pointer size %d failed to finalize nil: %v", pointerSize, ctx.Issues())
		}
	}
}
...
func TestBlockFinalizeWritesNegativeRelativePointer(t *testing.T) {
	...
	if got := int32(binary.LittleEndian.Uint32(block.Writer.Bytes())); got != -20 {
		t.Fatalf("relative pointer = %d, want -20", got)
	}
	if len(pointerOffsets) != 1 || pointerOffsets[0] != 100 {
		t.Fatalf("pointer offsets = %v, want [100]", pointerOffsets)
	}
}

func TestBlockFinalizeReportsInvalidPointers(t *testing.T) {
	t.Run("unresolved target", func(t *testing.T) {
		...
		if got := binary.LittleEndian.Uint32(block.Writer.Bytes()); got != 0 {
			t.Fatalf("unresolved pointer = %d, want zero", got)
		}
	})
